...
export const NavSwitch = ({ inline }) => {
  ...
  return (
    <div
      className={`scroll-bar inline-block overflow-hidden mt-4 w-full xl:h-60 xl:overflow-y-auto border-t border-b border-zinc-400 dark:border-zinc-500 ${
        inline
      ...
      {links.map((link) => (
        <Link
          ...
          className={`xl:p-2 w-full flex duration-100 !no-underline ${
            isActive(link)
              ? 'xl:bg-gradient-to-b from-sky-600 via-sky-700 xl:bg-sky-800 xl:bg-opacity-50 border-b-[3px] border-amber-600 xl:border-none'
